# WellKenz Bakery ERP System - Database Schema Analysis

Based on the authentication view file (`resources/views/Auth/login.blade.php`) and the AuthController, I've analyzed the requirements for the complete PostgreSQL database schema for the WellKenz Bakery ERP system.

## Authentication Requirements Analysis

From the view file and controller, the system requires:

1. **Email-based authentication** (line 120 in login.blade.php: `name="email"`)
2. **Password field** (line 146 in login.blade.php: `name="password"`)
3. **Remember me functionality** (line 166 in login.blade.php)
4. **Role-based access control** with 5 roles:
   - admin
   - supervisor  
   - purchasing
   - inventory
   - employee (Baker)
5. **Account status tracking** (lines 33-35 in AuthController: checking `is_active`)
...
CREATE TABLE users (
    ...
    password_hash VARCHAR(255) NOT NULL, -- Laravel Hash::check compatible
...
-- ============================================================================
-- SESSIONS TABLE (Laravel Session Storage)
-- ============================================================================
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'purchase_order_items' THEN
        ...
        grand_total = (
            SELECT COALESCE(SUM(total_price), 0) + tax_amount - discount_amount
            FROM purchase_order_items
            WHERE purchase_order_id = NEW.purchase_order_id
        )
...
1. **Complete Authentication System**: Supports the email/password login with role-based access control as shown in the view file
2. **Inventory Management**: Full stock tracking with batch management, expiry dates, and movement tracking
3. **Procurement System**: Purchase requests, purchase orders, and supplier management
4. **Production Management**: Recipe management and production order tracking
5. **Requisition System**: Internal material request workflow
6. **Audit Trail**: Complete activity logging for compliance
7. **System Settings**: Configurable system parameters
8. **Notifications**: User notification system
9. **Automated Calculations**: Triggers for automatic stock updates and total calculations

The schema is designed to be production-ready with proper indexing, referential integrity, and business logic enforcement through database constraints.
