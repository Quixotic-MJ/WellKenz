<div id="createItemModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white max-w-2xl w-full rounded-lg border border-gray-200">
        <form method="POST" action="{{ url('/admin/items') }}">@csrf
            ...
            <div class="p-6 grid grid-cols-2 gap-4 text-sm">
                ...
                <div><label class="block font-medium text-gray-700 mb-1">Category</label>
                    <select name="cat_id" required class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400">
                        @foreach($categories as $cat)<option value="{{ $cat->cat_id }}">{{ $cat->cat_name }}</option>@endforeach
                    </select>
                </div>
                <div><label class="block font-medium text-gray-700 mb-1">Unit</label><input type="text" name="item_unit" required class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400"></div>
                <div><label class="block font-medium text-gray-700 mb-1">Current Stock</label><input type="number" name="item_stock" required min="0" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400"></div>
                ...
                <div><label class="block font-medium text-gray-700 mb-1">Expiry Date</label><input type="date" name="item_expire_date" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400"></div>
                <div class="col-span-2"><label class="block font-medium text-gray-700 mb-1">Description</label><textarea name="item_description" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400"></textarea></div>
            </div>
