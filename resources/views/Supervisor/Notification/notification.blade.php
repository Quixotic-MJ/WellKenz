...
<div class="space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Supervisor Notifications</h1>
                <p class="text-sm text-gray-500 mt-1">Important updates and alerts – mark as read when done</p>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5">
            ...
            <p class="text-2xl font-semibold text-gray-900 mt-2">{{ $todayCount }}</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg">
            ...
            @forelse($notifications as $n)
                ...
                <div class="notif-row flex items-start p-6 {{ !$n->is_read ? 'bg-amber-50 hover:bg-amber-100' : 'bg-white hover:bg-gray-50' }} transition"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-600 mt-1">{{ \Illuminate\Support\Str::limit($n->notif_content, 150) }}</p>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            @if(!$n->is_read)
                                <button onclick="markRead({{ $n->notif_id }})"
                                    class="p-2 text-green-600 hover:bg-green-50 rounded-full transition" title="Mark read">
        ...
        <div class="px-6 py-3 border-t border-gray-200 bg-gray-50 text-xs text-gray-500">
            Showing <span id="visibleCount">{{ $notifications->count() }}</span> of {{ $notifications->total() }}
            notifications
        </div>
    ...
    @include('Supervisor.Notification.view')
...
<script>
    ...
    function filterModule(val) {
        ...
        rows.forEach(r => {
            ...
            r.style.display = ok ? 'flex' : 'none'; // <-- Use 'flex'
            if (ok) visible++;
        });
    ...
    function sortTable(f) {
        ...
        const rows = Array.from(tbody.querySelectorAll('.notif-row')); // <-- Use '.notif-row'
        rows.sort((a, b) => {
            const A = a.dataset[f],
                B = b.dataset[f];
    ...
    function openViewModal(id) {
        ...
        .then(data => {
            if (data.success) {
                ...
                body.innerHTML = `
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-900 font-semibold">${n.notif_title}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Content</label>
                            <p class="text-gray-900 bg-gray-50 p-3 rounded">${n.notif_content}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Module</label>
                                <p class="text-gray-900">${n.related_type ? n.related_type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'N/A'}</p>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <span class="inline-block px-2 py-1 text-xs font-semibold rounded ${n.is_read ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}">${n.is_read ? 'Read' : 'Unread'}</span>
                            </div>
                            <div>
                ...
                // Mark the row as read visually without a full page reload
                const row = document.querySelector(`.notif-row[data-date="${n.created_at_raw_for_matching}"]`); // You would need to pass this from controller
                if (row) {
                    row.classList.remove('bg-amber-50', 'hover:bg-amber-100');
                    row.classList.add('bg-white', 'hover:bg-gray-50');
                    row.dataset.read = 'read';
                    const markReadButton = row.querySelector('button[onclick^="markRead"]');
                    if (markReadButton) markReadButton.remove();
                }
            } else {
                showMessage(data.message || 'Error loading notification details', 'error');
            }
        })
        .catch(() => showMessage('Error loading notification details', 'error'));
    }
</script>
@endsection