        ...
        <div class="bg-white border border-gray-200 rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-3">
                    <select onchange="filterModule(this.value)"
                    ...
                    <select onchange="filterRead(this.value)"
                        class="text-sm border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-gray-400">
                ...
                <table class="w-full text-sm" id="notificationsTable">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase cursor-pointer"
                                onclick="sortTable('date')">Timestamp <i class="fas fa-sort ml-1"></i></th>
                    ...
                    <tbody class="divide-y divide-gray-200" id="notificationsTableBody">
                        @foreach ($notifications as $n)
                            <tr class="hover:bg-gray-50 transition notif-row" data-module="{{ $n->related_type }}"
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ ucfirst(str_replace('_', ' ', $n->related_type)) }}</td>
                                <td class="px-6 py-4 text-sm font-semibold text-gray-900">{{ $n->notif_title }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ \Illuminate\Support\Str::limit($n->notif_content, 60) }}</td>
                                <td class="px-6 py-4">
                                    @if ($n->is_read)
                                        <span
                                            class="inline-block px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded">Read</span>
                                    @else
                                        <span
                                            class="inline-block px-2 py-1 bg-amber-100 text-amber-700 text-xs font-semibold rounded">Unread</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
            ...
            <div class="px-6 py-3 border-t border-gray-200 bg-gray-50 text-xs text-gray-500">
                Showing <span id="visibleCount">{{ $notifications->count() }}</span> of {{ $notifications->count() }}
                notifications
            </div>
        </div>
                ...
                body.innerHTML = `
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-gray-900 font-semibold">${data.notif_title}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Content</label>
                            <p class="text-gray-900 bg-gray-50 p-3 rounded">${data.notif_content}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Module</label>
                                <p class="text-gray-900">${data.related_type ? data.related_type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'N/A'}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Related ID</label>
                                <p class="text-gray-900">${data.related_id || 'N/A'}</p>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                                <span class="inline-block px-2 py-1 text-xs font-semibold rounded ${data.is_read ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}">${data.is_read ? 'Read' : 'Unread'}</span>
                            </div>
                            <div>
