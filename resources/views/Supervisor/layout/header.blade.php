...
<script>
    ...
    async function loadHeaderNotifications() {
        ...
        try {
            ...
            const response = await fetch('{{ route("supervisor.notifications.header") }}', {
                headers: {
                ...
                if (notifications.length === 0) {
                    ...
                    listEl.innerHTML = notifications.map(notification => {
                        ...
                        const containerClass = isRead 
                            ...
                            : 'bg-blue-50/40 hover:bg-blue-50/70 border-l-4 border-l-caramel';
                        ...
                        if (notification.type === 'approval_req' || notification.type === 'purchasing') {
                            ...
                            categoryColor = 'text-green-600 bg-green-50 border-green-100';
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex justify-between items-start mb-1">
                                            <div class="flex items-center gap-2">
                                                <span class="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border ${categoryColor}">${categoryLabel}</span>
                                                <span class="text-[10px] text-gray-400">•</span>
                                                <span class="text-[10px] text-gray-400 font-medium">${notification.priority}</span>
                                            </div>
                                            <span class="text-[10px] text-gray-400 whitespace-nowrap font-medium uppercase tracking-wide">${notification.time_ago}</span>
                                        </div>
                    ...
                    if (summaryEl && notifications.length > 0) {
                        ...
                        summaryEl.innerHTML = `
                            <span class="font-medium">${unread} unread</span> • 
                            <span class="text-gray-400">Last updated ${new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        `;
                    }
                }
    ...
    async function handleNotificationClick(notificationId, actionUrl) {
        try {
            ...
            if(item) {
                item.classList.remove('bg-blue-50/40', 'hover:bg-blue-50/70', 'border-l-4', 'border-l-caramel');
            ...
            await fetch(`{{ route('supervisor.notifications.mark-read', ['notification' => '__ID__']) }}`.replace('__ID__', notificationId), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    async function markAllAsRead() {
        try {
            const response = await fetch('{{ route("supervisor.notifications.mark-all-read") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json'
    ...
    // Restore sidebar state and load notifications
    ...
    // Separate function for checking count without opening
