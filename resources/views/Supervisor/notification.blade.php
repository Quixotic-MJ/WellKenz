...
<div class="w-full px-4 sm:px-6 lg:px-8 space-y-8 pb-24 font-sans text-gray-600 relative">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <h1 class="font-display text-3xl font-bold text-chocolate mb-1">Notifications</h1>
            <p class="text-sm text-gray-500">Stay updated with production alerts, stock levels, and system messages.</p>
        </div>
        <div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 flex-wrap">
                                    ...
                                    {{-- Priority Badge --}}
                                    @if(isset($notification->priority) && $notification->priority !== 'normal')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTabCounts() {
        // This logic needs to fetch or calculate. Assuming you might implement an AJAX call here.
        // For now, re-counting DOM elements for instant feedback if not paginated fully.
        // In a real app, you'd likely fetch updated counts from an API endpoint.
        /* fetch('/supervisor/notifications/counts')
            .then(r => r.json())
    ...
    document.querySelectorAll('.notification-item').forEach(item => {
        ...
        if (markReadUnreadBtn) {
            markReadUnreadBtn.addEventListener('click', function() {
                ...
                fetch(`/supervisor/notifications/${notificationId}/mark-${action}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', function() {
                openConfirmModal('Delete Notification', 'This action cannot be undone.', function() {
                    fetch(`{{ route('supervisor.notifications') }}/${notificationId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
    ...
    function performBulk(operation) {
        ...
        openConfirmModal(title, msg, function() {
            fetch('{{ route("supervisor.notifications.bulk-operations") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
