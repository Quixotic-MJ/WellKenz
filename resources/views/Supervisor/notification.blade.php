...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', function() {
                openConfirmModal('Delete Notification', 'This action cannot be undone.', function() {
                    // Actual API call
                    fetch(`/supervisor/notifications/${notificationId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json',
    ...
    function performBulk(operation) {
        ...
        openConfirmModal(title, msg, function() {
            // Actual API call for bulk operations
                ...
                if (data.success) {
                    ...
                    if (operation === 'delete') {
                        ...
                        document.querySelectorAll('.notification-checkbox:checked').forEach(cb => {
                            ...
                            if (h3) h3.classList.remove('text-chocolate');
                            const dot = h3?.nextElementSibling;
                            if (dot && dot.tagName === 'SPAN' && dot.classList.contains('bg-chocolate')) dot.remove();
                            const btn = item.querySelector('.mark-read-unread');
                            if (btn) btn.textContent = 'Mark Unread';
                        });
                    ...
                    showToast('Error', data.error || 'Failed to process bulk operation', 'error');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                showToast('Error', 'An error occurred while processing bulk operation', 'error');
            });
        });
    }
