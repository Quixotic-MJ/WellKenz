                        ...
                        <tr class="hover:bg-gray-50 transition-colors {{ $hasHighRequest && !$hasSufficientStock ? 'hover:bg-red-50 border-l-4 border-l-red-400' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    ...
                                    @if($mainItem)
                                        <button 
                                            onclick="RequisitionManager.openModifyModal({{ $requisition->id }}, {{ $mainItem->id }}, '{{ $mainItem->item->name ?? 'Unknown Item' }}', {{ $requestedQty }}, '{{ $mainItem->item->unit->symbol ?? '' }}', {{ $currentStock }})" 
