...
<div class="space-y-8 font-sans text-gray-600" id="stockCardApp">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('supervisor.inventory.stock-level') }}" class="flex items-center justify-center w-10 h-10 bg-white border border-border-soft rounded-xl text-chocolate hover:bg-cream-bg transition-colors shadow-sm group">
        ...
        <div class="px-6 py-4 border-b border-border-soft bg-cream-bg flex flex-col lg:flex-row justify-between items-center gap-4">
            ...
            <form method="GET" class="flex items-center gap-2 w-full lg:w-auto">
                ...
                @if(request('date_from') || request('date_to'))
                    <a href="{{ route('supervisor.inventory.stock-card', $item) }}" class="px-3 py-2 text-xs font-bold text-gray-400 hover:text-red-500 transition-colors">
