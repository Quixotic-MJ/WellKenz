...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">Inventory Notifications</h1>
                    <p class="text-sm text-gray-500 mt-1">Alerts on deliveries, issuances, stock & expiry – mark as read when
                        done</p>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-200" id="notificationsTableBody">
                        @foreach ($notifications as $n)
                            <tr class="hover:bg-gray-50 transition notif-row cursor-pointer"
                                data-module="{{ $n->related_type }}" data-read="{{ $n->is_read ? 'read' : 'unread' }}"
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <a href="{{ route('inventory.notifications.jump', $n->notif_id) }}"
    ...
    <!-- ====== VIEW MODAL  ====== -->
    @include('Inventory.Notification.view')
...
@push('scripts')
    <script>
        ...
        function markReadInline(id, row) {
            ...
            fetch(`/inventory/notifications/${id}/mark-read`, {
                    method: "POST",
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function openViewModal(id) {
            fetch(`/inventory/notifications/${id}`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                })
                ...
                .then(res => {
                    ...
                    if (!res.is_read) fetch(`/inventory/notifications/${id}/mark-read`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
