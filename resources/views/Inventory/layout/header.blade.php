<header class="bg-white border-b border-border-soft sticky top-0 z-30 font-sans shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)]">
        ...
        <div class="flex items-center space-x-5">
            ...
            <div class="relative" role="menu">
                ...
                <div id="notificationsDropdown" class="hidden absolute right-0 mt-4 w-80 sm:w-96 bg-white shadow-2xl border border-border-soft z-50 rounded-2xl overflow-hidden origin-top-right transition-all duration-200 transform">
                    ...
                    <a href="{{ route('inventory.notifications') }}" class="block p-4 text-center text-xs font-bold text-white bg-chocolate hover:bg-chocolate-dark transition-all uppercase tracking-widest">
                        View All Activity
                    </a>
                </div>
            </div>
            ...
            if (data.success) {
                ...
                if (notifications.length === 0) {
                    ...
                    listEl.innerHTML = notifications.map(notification => {
                        // Default Icon Handling if not provided by backend
                        const iconParts = (notification.icon_class || 'fas fa-bell').split(' ');
